module.exports = (app) => {
  ...
  const save = async (req, res) => {
    ...
    try {
      ...
      existsOrError(fornecedor.cnpj, "cnpj não informado!");
      existsOrError(fornecedor.email, "E-mail não informado!");
      existsOrError(
        fornecedor.enderecoId,
        "Endereço do fornecedor não informado!"
      );

      // const fornecedorFromDB = await app
      //   .db("fornecedor")
  ...
  const remove = async (req, res) => {
    try {
      const fornecedorCompra = await app
        .db("compra")
        .where({ fornecedorId: req.params.id });
      notExistsOrError(
        fornecedorCompra,
        "Compra ja foi cadastrada com esse Fornecedor."
      );
