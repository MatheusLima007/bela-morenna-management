module.exports=app=>{
    ...
    const get = async (req, res)=>{
        ...
        app.db({p: 'produto', t: 'tamanho'})
            .select('p.id', 'p.descricao', 'p.quantidade', 'p.marca', 't.descricao as descricaoTamanho')
    ...
    const remove = async (req, res)=>{
        try{
            ...
            notExistsOrError(produtoProdutoVenda, 'Venda ja foi cadastrada com esse Produto.')

            const produtoProdutoCompra = await app.db('produto_compra')
                .where({ produtoId: req.params.id })
            notExistsOrError(produtoProdutoCompra, 'Compra ja foi cadastrada com esse Produto.')

            const rowsDeleted = await app.db('produto')
